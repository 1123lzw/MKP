...
public class GGAHelper extends Individual {
    //权重表转换
    ...
    public boolean isValidSingle(int[] code,double[] constrain) {
        boolean flag = false;
        double limit = constrain[constrain.length - 1];
        double cur_val = count(code, constrain);
        if (cur_val > limit)
            flag = false;
        else
            flag = true;
