...
public class Individual implements Comparable<Individual>{
    ...
    public Individual cross(EAHMP.FPTree fpgafpTree) {  //交叉  相同的就确定
        if (random == null) {
            random = new Random();
        }
    ...
    public void repair(int[] code) {  //修复
        double[] value = mkpProb.getValueVectortran();
        EAHMP.Helper particleHelper = new EAHMP.Helper();
    ...
    public void add(int[] p, double[][] constraint, int[] code) {
        EAHMP.Helper particleHelper = new Helper();
        boolean flag;
        int dimension = p.length;
