...
public class CondPattBase {
     ...
     *
     */
    public double[] comRatio(MKPProb mkpProb) {  //Profit density
        double[] value=mkpProb.getValueVectortran();
        double[][] constrain=mkpProb.getContraintMatrixtran();
        double[] r = new double[value.length];
        for (int j = 0; j < value.length; j++) {
            double c =0;
            for (int i = 0; i < constrain.length; i++) {
                c += constrain[j][i]/constrain[value.length][i];
            }
            r[j] = value[j]/(c/constrain.length);
        }
    ...
    public ArrayList<ArrayList<Item>> loadItemSets(ValueTbl tbl, MKPProb solveProb) {
        ArrayList<ArrayList<Item>> origItemSets = new ArrayList<ArrayList<Item>>();  //标注<编号，次数，总价值，伪功效比>的表
        ArrayList<ValueTableItem> ValueTableItems = tbl.getValueTableItems(); //事务
        double[] ratio=comRatio(solveProb);

        for (MFUC.ValueTableItem ValueTableItem : ValueTableItems) {
            // 选用的编译选项编号的集合
            ArrayList<Integer> optNums = ValueTableItem.getOptNums();
            ArrayList<Item> itemSet = new ArrayList<Item>();
            double objVal = ValueTableItem.getObjVal();
            for (Integer optNum : optNums) {  //<编号，次数，价值,物品伪功效比>
                Item item = new Item(String.valueOf(optNum), 1, objVal,ratio[optNum]);
                itemSet.add(item);
            }
